...
# Time complexity
# O(N ^ (T/M)): the depth is T/M, at each step, we have N choice
# Space: O(N ^ (T/M))
# The code uses a backtracking algorithm to find all possible combinations. 
# The time complexity can be analyzed by considering the decision tree generated by the recursive calls.

# Let N be the number of candidates (len(candidates)).

# Let T be the target value.

# Let M be the minimum value in the candidates list.

class Solution:
    def combinationSum(self, candidates: List[int], target: int) -> List[List[int]]:
        res = []
        
        def backtrack(start_index, cur_comb, target_sum):
            if target_sum == 0:
                res.append(cur_comb[:])
                ...
                for i in range(start_index, len(candidates)):
                    ...
                    backtrack(i, cur_comb, target_sum - candidate)
                    cur_comb.pop()
